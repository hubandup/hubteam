...
interface AddActivitySectorDialogProps {
  onSectorAdded: () => void;
}

export function AddActivitySectorDialog({ onSectorAdded }: AddActivitySectorDialogProps) {
  ...
  const onSubmit = async (data: SectorFormData) => {
    ...
    try {
      ...
      toast.success('Secteur d\'activité ajouté');
      setOpen(false);
      reset();
      onSectorAdded();
    } catch (error: any) {
      ...
      if (error.code === '23505') {
        toast.error('Ce secteur existe déjà');
      } else {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Créez un nouveau secteur d'activité personnalisé
          </DialogDescription>
        </DialogHeader>
