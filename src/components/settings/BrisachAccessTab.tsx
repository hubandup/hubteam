...
export function BrisachAccessTab() {
  const queryClient = useQueryClient();
  const [search, setSearch] = useState('');

  const { data: users, isLoading } = useQuery({
    queryKey: ['brisach-access-management'],
    queryFn: async () => {
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['brisach-access-management'] });
      queryClient.invalidateQueries({ queryKey: ['brisach-access'] });
    },
    onError: () => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Gérez les utilisateurs ayant accès au dashboard Brisach. Les administrateurs et l'équipe apparaissent aussi ici avec leur accès automatique.
        </p>
      </div>
