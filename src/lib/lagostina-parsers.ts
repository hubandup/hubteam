...
export async function parseMediaFile(workbook: XLSX.WorkBook) {
  ...
  // SEA (KPIs + Top Keywords + Campagnes) est exclusivement synchronisé depuis Google Sheets
  // (edge function sync-google-ads-sea). On exclut donc le canal "sea" du parsing kDrive
  // et on préserve les lignes SEA déjà présentes en base.
  const final = Array.from(merged.values()).filter((r) => r.channel !== 'sea');

  await supabase.from('lagostina_media_kpis').delete().neq('channel', 'sea');
